...
| `diretta.it` | ⚠️ **non praticabile senza reverse-engineering**: `robots.txt` per `*` blocca solo `/classifiche/`, `/tabellone/`, `/newsfeed/` (nessuna regola specifica per `ClaudeBot`/`anthropic-ai`/`GPTBot`), ma è una SPA — risultati/quote sono caricati via JS/XHR e non compaiono nell'HTML statico fetchato. Andare oltre richiederebbe chiamare le API interne non documentate: zona grigia diversa dalla "lettura da browser" già accettata per i valori rosa (R2) |
