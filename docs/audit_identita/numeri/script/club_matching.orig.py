...
def canonico(nome: str) -> str:
    """Come `normalizza`, ma CONSERVA l'ordine dei token."""
...
_BLOCCO_CANONICO: frozenset[str] = frozenset(canonico(x) for x in NON_AGGANCIARE)
...
class Agganciatore:
    ...
    def candidati(self, nome: str) -> list[int]:
        ...
        can = canonico(nome)
        if can and can in _BLOCCO_CANONICO:
            return []
        ts = normalizza(nome)
