...
| **87** | **coda a 2 parametri, riprodotta** (`_run_tail_two_param.py`): isotonica per-soglia + mistura di due Poisson, walk-forward | **(A) isotonica**: peggiora il log-loss OOS su **tutte e 4** le soglie (Over1.5 +0.0150 … Over4.5 +0.0109); **(B) mistura**: guadagno in-sample (s=0.15, −0.0006) ma **OOS non conclusivo** (Δ −0.00042, CI [−0.0015,+0.0006], P 78.6%) e **segno ribaltato sulle stagioni recenti** (2425/2526 positive) | ❌ entrambe chiuse; la coda-forma è al tetto (3ª conferma dopo COM-Poisson e θ_team) |
| **88** | **handicap asiatico = benchmark Tier 2** (`_run_ah_benchmark.py`, 7.437 partite × 3 leghe): il router prezza la copertura del margine come il mercato sharp? | **Brier router ≈ Brier mercato** (0.2040 vs 0.2041 aggregato, indistinguibili a coppie per lega), corr modello-mercato **0.91**; dai soli λ,μ del 1X2+O/U il motore eguaglia il mercato che quota l'AH direttamente | ✅ **α*=0 su un mercato NUOVO** (il margine); primo test Tier 2, router validato per copertura/scarto |
...
| **91** | **mercati POSIZIONALI dal `rank` gia' calcolato** (leva aperta dall'audit F90): P(top-4) e P(retrocessione) — **480 osservazioni binarie** per mercato invece delle 24 del campione, zero modellistica nuova | **TOP-4: ottimamente calibrato** — log-loss **0.2218**, Brier 0.0675, **ECE 0.0137** (scarto max **1.4pp** su tutte le fasce); batte il tasso base +0.2786 [+0.2208,+0.3345] E la persistenza +0.0273 [+0.0037,+0.0502], entrambi **conclusivi**. **RETROCESSIONE: rotta** — batte il tasso base (+0.0875) ma **non la persistenza** (−0.0116, IC contiene 0), e la calibrazione crolla in alto: dichiarando 70-90% succede il **46.7%**, dichiarando >90% il **60.0%**. **Colpevole isolato**: dei 37 casi sopra il 60%, **36 sono neopromosse** (97%) e 19 si sono salvate — neopromosse dichiarato 54.7% vs realizzato 48.6% (−6.1pp, era −10.1pp prima del fix del prior alla F92), **resto della lega calibrato** (7.3% vs 9.1%). Il prior δ, tarato sul log-loss della SINGOLA partita (F7/57), e' troppo severo propagato su 38 giornate | ✅ due mercati nuovi, uno dei quali (top-4) e' il risultato meglio calibrato del progetto su questa famiglia; 🔎 pista concreta: **δ dipende dall'orizzonte** — ritararlo sul bersaglio stagionale |
...
deficit è **104% informazione e −4% calibrazione** (siamo *meglio* calibrati del
mercato, 0.00083 contro 0.00125, e perdiamo tutta la risoluzione), concentrato
