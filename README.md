...
| **140** | **le coppe imparano a dividere i due tempi** — l'utente consegna le STATISTICHE di Carabao Cup e FA Cup 2025-26 (coppe già in casa dalla F139-bis/ter); nuova porta d'ingresso `scripts/registra_raccolta_statistiche_coppa_diretta.py` | **Le righe erano identiche, il valore stava tutto nelle COLONNE.** Tre cose nuove: ⭐ le **45 statistiche di squadra per PERIODO** (Totale / 1° / 2° tempo, + Supplementari), che per le coppe non esistevano — è il dato che serve al modello a due stadi (residuo aperto F96/99); ⭐ la colonna **`ID partita`** sulle statistiche per giocatore, che `aggancia_coppe.py` documentava a riga 239 di dover **aggirare** appaiando per `(Data, Casa, Ospite)` e poi per nome; e **6 decimali invece di 3**. Tutto verde e i conti chiudono esatti: additività dei periodi **7.098/7.098** (Carabao) e **4.914/4.914** (FA Cup, con 3 addendi sulle 14 partite ai supplementari); fedeltà CSV↔`.xlsx` **328.325/328.325** e **227.010/227.010**; punteggi vs raccolta già in casa **91/91** e **63/63**; **0 divergenze reali** su 322.615 + 223.062 celle, scarto massimo **0.0005** — esattamente il tetto dell'arrotondamento a 3 decimali, contro una soglia di stop 10× più larga. Tre controlli **semantici indipendenti** che nessuna verifica impone: 1.638 celle composite riespanse a mano tutte coincidenti, **possesso palla = 100** su 91/91 partite, `Risultato squadra` coerente su 182/182. ⭐ **Il difetto era nel CONTROLLO, non nei dati**: la prima esecuzione accusava «134 celle divergenti» su `Rating` mentre i due file erano identici — da **pandas 3.0** `astype(str)` lascia il NaN com'è invece di produrre `'nan'`, e `NaN != NaN` | ✅ terza porta d'ingresso, 23 test nuovi; ⚠️ **raccolto, non usato** — nessun modello legge queste colonne (§5-ter); ⚠️ anomalia dichiarata (R4): `EFL Cup` e `Carabao Cup` sono la stessa competizione, il manifesto ora lo scrive |
