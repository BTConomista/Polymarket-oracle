...
| **45** | **router stakes-aware** (path senza quote: DC ovunque, GBM-stakes sul mismatch una-decisa/una-in-corsa) | gap mismatch DC vs mercato **+0.0549** (conferma Fase 31); ma **GBM-stakes 1.0087 > DC 0.9943** anche sul mismatch (il "6× meglio" della Fase 32 era vs GBM-base, non vs DC); router hard +0.0145, soft −0.0018 (CI [−0.034,+0.028], P 47%) | ❌ segnale reale ma **non sfruttabile**; chiude l'ultimo lead predittivo interno (info del mercato, non nostro errore) |
| **46** | **ensemble standalone** (DC + bivariato + GBM per-mercato, mean/logpool/DC+GBM) | miglior singolo = bivariato (≈DC, +0.0003); su 1X2 l'ensemble **peggiora** (mean +0.0033, dc_gbm +0.0080: il GBM zavorra); su O2.5/GG neutro ma nel rumore (P 23–34%) | ❌ nessun ensemble batte il singolo: DC≈biv (no diversità), GBM diverso ma peggiore |
