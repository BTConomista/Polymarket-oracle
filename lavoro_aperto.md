...
### B · Lavori concreti, nessuna decisione richiesta (7)
...
| B6 | 🔎 **Le 8 partite di Europa League che non ricompongono il punteggio** — reperto della Fase 158-bis, **misurato e NON diagnosticato** | poche ore | Con l'identità `Gol = 1T + 2T + suppl.` corretta, la Conference sbaglia su **esattamente** le 6 partite con la lotteria dei rigori (difetto noto e riparato) e la Champions su **zero**. L'Europa League su **15**: 7 con la lotteria, **8 no** — e su **7 di quelle 8 tutte e quattro le colonne dei tempi valgono ZERO mentre la partita ha dei gol**. È uno zero che significa «non lo so» (R6), non un errore di somma. ⚠️ Non è nuovo: c'era da prima, e stava nascosto in mezzo ai 32 falsi allarmi che la guardia a due addendi produceva. Va istruito **a mano, partita per partita** (R5, «spiegare prima di accusare») prima di dire cosa sia; finché non lo è, `punteggio_vero()` sulle UEFA resta affidabile sulla lotteria e **muto** su questo. Elenco riproducibile: `tf.squadre("uefa_europa_league", periodo="Totale")` filtrato con `~tf.gol_sono_regolamentari(...)` |
| B7 | 📚 **Uno snapshot per le due seconde divisioni appena raccolte** (Fasi 158 / 158-bis) | mezza giornata | `files/tre_fonti_{laliga2,bundesliga2}_2526/` sono **complete e verificate** ma non hanno né quote né `data/{lega}_matches.csv`, quindi nessun modello può leggerle. football-data pubblica `SP2` e `D2` con schema **identico** alle prime divisioni, quote di chiusura comprese. È il passo che trasformerebbe due raccolte archiviate in due leghe modellabili. ⚠️ Con una trappola già misurata: dei 22 club spagnoli **12 non compaiono in nessuno snapshot** e i loro nomi non sono mai stati messi in corrispondenza con la grafia football-data — e la lezione delle due fasi è che **l'euristica sugli accenti non basta** (`Deportivo de A Coruña`→`La Coruna`, `Fürth`→`Fuerth`: in un caso la lingua cambia, nell'altro la traslitterazione *espande* la vocale) |
